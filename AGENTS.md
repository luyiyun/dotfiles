...
- `shell/`：`stow shell` 后部署为 `~/.config/shell/common.sh`
- `zsh/`：`stow zsh` 后部署为 `~/.zshrc`
...
- Bash 和 Zsh 共用 `shell/.config/shell/common.sh`
- 共享文件中只放两者都适用的环境变量与函数
- shell 专属启动逻辑仍留在各自的 `.bashrc` / `.zshrc`
...
- 已改为通过共享文件统一维护 `PATH`、locale、proxy 和 tmux 相关环境变量。
- 若新增 alias 或环境变量，优先放在文件底部自定义区域，避免打乱系统默认模板段落。

### `.zshrc`

- 当前使用 Oh My Zsh。
- theme 为 `random`，插件包含 `git`、`colorize`、`zsh-autosuggestions`、`zsh-syntax-highlighting`。
- 若新增只适用于 Zsh 的逻辑，请保留在 `zsh/.zshrc`，不要放进共享文件。
...
- Shared shell 配置检查：`sh -n shell/.config/shell/common.sh`
- Zsh 配置检查：`zsh -n zsh/.zshrc`
- Git 配置检查：`git config --file git/.gitconfig --list`
- tmux 配置检查：`tmux source-file tmux/.tmux.conf`
- Stow 预演：`stow -n -v shell bash zsh git tmux`
